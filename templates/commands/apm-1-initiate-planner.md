...
2. State the Planning Phase is complete: planning documents created, Message Bus initialized. Direct the User to open a new terminal and initiate the Tech Lead with `/apm-tech-lead` in the same AI assistant that ran this Planning Phase. The Tech Lead performs the architectural review and adds operational guardrails to Rules before implementation begins. After the Tech Lead completes its review, the User initiates the Manager with `/apm-2-initiate-manager` in a new chat to start the Implementation Phase.
